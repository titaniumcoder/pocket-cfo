...
// TestTheBalanceSaysWhereItCameFrom: three ways a figure can be arrived at —
// read straight off the bank, carried on imported statements, carried on the
// budget — and the reader cannot tell them apart from the number alone. The
// tooltip is the whole explanation, so it has to name the source and, when
// the chain mixes the two, say how much of it was which.
func TestTheBalanceSaysWhereItCameFrom(t *testing.T) {
	ctx := context.Background()
	full := augustActuals("2026-08-01", "2026-08-31")
	...
	tests := []struct {
		...
		{
			"the month the reading opens is the reading",
			time.August, full,
			"Read off the bank on 31 July 2026.",
		},
		{
			"one month carried on the statements that cover it",
			time.September, full,
			"Read off the bank on 31 July 2026, then carried 1 month, each closed on the imported statements.",
		},
		{
			"no statements at all, so the budget carried it",
			time.October, nil,
			"Read off the bank on 31 July 2026, then carried 2 months, each closed on the budget.",
		},
		{
			"August imported, September not",
			time.October, full,
			"Read off the bank on 31 July 2026, then carried 2 months — 1 closed on the imported statements, 1 on the budget.",
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			f := actualsTracker(t, tt.actuals).ComputeMonth(ctx, 2026, tt.month)
			if f.BalanceBasisNote != tt.want {
				t.Errorf("basis note =\n  %q\nwant\n  %q", f.BalanceBasisNote, tt.want)
			}
		})
	}
}

// TestTheBasisReachesThePageAsATooltip: the note is only useful attached to
// the figure it explains, and it is deliberately not written out as a row.
func TestTheBasisReachesThePageAsATooltip(t *testing.T) {
	f := actualsTracker(t, nil).ComputeMonth(context.Background(), 2026, time.October)
	if f.BalanceBasisNote == "" {
		t.Fatal("no basis note to render")
	}
	rec := httptest.NewRecorder()
	RenderPage(rec, f)
	body := rec.Body.String()

	if !strings.Contains(body, `class="basis" title="`+f.BalanceBasisNote+`"`) {
		t.Errorf("the opening balance carries no basis tooltip:\n%s", body)
	}
	if strings.Contains(body, `<span class="stale-note">`+f.BalanceBasisNote) {
		t.Error("the basis was written out as a note row — it belongs in the tooltip only")
	}
}
