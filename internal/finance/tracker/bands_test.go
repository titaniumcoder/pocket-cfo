...
func atGross(p PersonalParams, r Rules, gross float64) PersonalView {
	r.MinimumEUR = gross
	return p.breakdown(0, 0, 1, r, SalaryDecision{Mode: SalaryFull}, companyStock{}, noDividend)
}
...
func TestVectorTheMinimumWageBinding(t *testing.T) {
	p := bulgariaBands()
	v := p.breakdown(300, 0, 1, p.rulesFor(yearMonth{2026, time.July}), SalaryDecision{Mode: SalaryFull}, companyStock{}, noDividend)
...
func TestVectorZeroedPeriod(t *testing.T) {
	...
	v := p.breakdown(5000, 0, 1, p.rulesFor(yearMonth{2026, time.March}), SalaryDecision{Mode: SalaryFull}, companyStock{}, noDividend)
...
func TestTheLedgerColumnSubtractsExactly(t *testing.T) {
	...
	for _, income := range []float64{800, 1000, 1234.56, 2000, 2511.16, 5000, 10000} {
		v := p.breakdown(income, 0, 1, r, SalaryDecision{Mode: SalaryFull}, companyStock{}, noDividend)
		if v.MinimumEnforced {
...
func TestARangeShowsALinePerSchedule(t *testing.T) {
	...
	month := p.breakdown(1000, 0, 1, p.rulesFor(yearMonth{2026, time.June}), SalaryDecision{Mode: SalaryFull}, companyStock{}, noDividend)
