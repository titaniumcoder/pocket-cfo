...
<main>
  ...
  <section class="panel budget-panel">
    ...
    <div class="ledger{{if .ShowActuals}} with-actuals{{end}}">
      ...
      {{if and (not .Mode) (or .MonthsWithoutSalary .MonthsAtMinimum)}}<div class="row{{if and .CompanyGroups (not .NoLegislation)}} gap-above{{end}}"><span class="stale-note">{{if .MonthsAtMinimum}}{{.MonthsAtMinimum}} month(s) pay only the statutory minimum{{end}}{{if and .MonthsAtMinimum .MonthsWithoutSalary}}, and {{end}}{{if .MonthsWithoutSalary}}{{.MonthsWithoutSalary}} month(s) draw no salary at all{{end}} — so this total is not twelve full months.</span></div>{{end}}
