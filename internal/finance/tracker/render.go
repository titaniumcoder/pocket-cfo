...
<main>
  ...
  <section class="panel">
    ...
    {{if .Balances}}
    <h3>Balances</h3>
    <div class="cov-grid bal-grid">
      <span class="cov-head">Account</span><span class="cov-head col-secondary">Read</span><span class="cov-head bal-amt">Balance</span>
      {{range .Balances}}<span>{{.Name}}</span><span class="col-secondary">{{.AsOf}}</span><span class="bal-amt">{{eur .Cents}}</span>{{end}}
    </div>
    {{end}}
...
<main>
  ...
  <section class="panel income-panel">
    ...
    <div class="ledger">
      ...
      {{if .TogglQuotaNote}}<div class="row"><span class="stale-note">{{.TogglQuotaNote}}</span></div>{{end}}
      {{if .TogglPending}}
  ...
  <section class="panel budget-panel">
    ...
    <div class="ledger{{if .ShowActuals}} with-actuals{{end}}">
      ...
      {{if .DividendCents}}<div class="row{{if and .CompanyGroups (not .NoLegislation)}} gap-above{{end}}"><span class="label">Company profit tax</span><span class="mid rate">{{template "rateMid" .CompanyProfitTaxRate}}</span><span class="amt neg">{{if .CompanyProfitTaxCents}}&minus;{{end}}{{eur .CompanyProfitTaxCents}}{{template "rateNarrow" .CompanyProfitTaxRate}}</span></div>{{end}}
      <div class="row{{if and .CompanyGroups (not .NoLegislation) (not .DividendCents)}} gap-above{{end}}"><span class="label">Employer social</span><span class="mid rate">{{template "rateMid" .EmployerRate}}</span><span class="amt neg">{{if .EmployerContribCents}}&minus;{{end}}{{eur .EmployerContribCents}}{{template "rateNarrow" .EmployerRate}}</span></div>
      ...
      {{if .DividendCents}}<div class="row sub"><span class="label">Dividend{{if .OneDividendDay}} <small>({{.OneDividendDay}})</small>{{end}}</span><span class="mid"></span><span class="amt total goodamt">{{eur .DividendCents}}</span></div>
      {{if not .OneDividendDay}}{{range .DividendRows}}<div class="row acct"><span class="label">{{.Day}}</span><span class="mid"></span><span class="amt">{{eur .Cents}}</span></div>{{end}}{{end}}{{end}}
      ...
      {{if .DividendCents}}<div class="row"><span class="label">Dividend tax</span><span class="mid rate">{{template "rateMid" .DividendTaxRate}}</span><span class="amt neg">{{if .DividendTaxCents}}&minus;{{end}}{{eur .DividendTaxCents}}{{template "rateNarrow" .DividendTaxRate}}</span></div>{{end}}
